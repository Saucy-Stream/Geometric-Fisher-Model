...
class FisherGeometricModel() :
    def __init__(self, n, initial_position, size, alpha, Q, sigma_mut, duplication_rate, deletion_rate, mutation_rate) :
        ...
        self.genes = [self.create_first_gene()] # first gene in the genotype studied, it must be at least neutral to be kept.
    ...
    def create_first_gene(self):
        ...
        Parameters :
            None 
            Only parameters self defined in the class object are necessary for this function (see __init__)
        ...
        while s < 0 : # we consider that the first gene must be at least neutral or beneficial so that the organism survive
            gene = np.random.normal(0, self.sigma_mut, self.dimension) # We draw the gene as if it was a mutation of the starting point in the Standart FGM
    ...
    def duplication_deletion(self): # 4 sec
        ...
        if np.random.rand() < 0.5 : # randomly decide about the order between duplication and deletion
            ...
            indexes_dupl = set() # memorize the index of the copied genes
            if nb_dupl > 0:
                dupl = True
                for i in range(min(nb_dupl, nb_genes)):
                    index = np.random.randint(0, nb_genes)
                    while index in indexes_dupl :
                        index = np.random.randint(0, nb_genes) # find a gene which hasn't been copied until now.
    ...
    def evolve(self, time_step) : # 15 sec
...
# cProfile.run('main()', sort="tottime") # 15 sec 
main()
...
for init_pos in list_init :
    fgm = FisherGeometricModel(n_traits, init_pos, population_size, alpha, Q, sigma_mut, duplication_rate, deletion_rate, mutation_rate)
    memory, fitness, effects = fgm.evolve(n_generations)
...
results = {}
for mu in list_mu :
    fgm = FisherGeometricModel(n_traits, initial_position, population_size, alpha, Q, sigma_mut, duplication_rate, deletion_rate, mu)
    memory, fitness, effects, methods = fgm.evolve(n_generations)
    results[mu] = fitness

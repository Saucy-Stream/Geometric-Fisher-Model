...
class FisherGeometricModel() :
    def __init__(self, n, initial_position, size, alpha, Q, sigma_mut, duplication_rate, deletion_rate, mutation_rate, ratio) :
        ...
        self.genes = [self.create_first_gene(ratio)] # first gene in the genotype studied, it must be at least neutral to be kept.
    ...
    def evolve(self, time_step) : # 20 sec
        ...
        """
        memory = [self.init_pos, self.final_pos] # memorize the to first position, which are the initial phenotype of the individual and its phenotype after addition of the first gene
        fitness = []
        effects = []
        methods = []
        nb_genes = [1] # at the beginning, the individual only as one gene in the genome
        ...
        for i in range(time_step):
            ...
            if dupl or dele or mut : # if there was at least one modification in the genome
                ...
                s = self.fitness_effect(initial_fitness, new_fitness) # the fitness effect of the modification
                ...
                if np.random.rand() < pf : # the mutation is fixed
                    ...
                    methods.append(method) # remember the method(s) that created this modification
            ...
            nb_genes.append(len(self.genes)) # remember the number of genes in the genotype after this iteration

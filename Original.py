...
class FisherGeometricModel() :
    def __init__(self, n, initial_position, alpha, Q, sigma_mut, duplication_rate, deletion_rate, mutation_rate, ratio, method) :
        """
    ...
    def mutation_on_one_gene(self, list_genes):
        ...
        Parameters :
            ...
            dimension, sigma_mut, mutation_rate.
        ...
        """
        n = len(list_genes)
        nb_mut = np.random.poisson(self.mutation_rate * n) # Blanquart 2014 ; number of mutation to do. the rate is multiply by the size of the population so that it represent the number of mutation per generation.
    ...
    def mutation_on_every_genes(self, list_genes):
        ...
        """
        nb_mut = np.random.poisson(self.mutation_rate) # Blanquart 2014 ; number of mutation to do. the rate is multiply by the size of the population so that it represent the number of mutation per generation.
        n = len(list_genes)
        if nb_mut > 0 : 
    ...
    def deletion(self):
        ...
        n = len(self.genes)
        list_genes = self.genes.copy() # make a copy of the initial list of genes so that it is not changed if the modification are not fixed afterward
        nb_del = np.random.poisson(self.deletion_rate*n) # number of deletion to do. the rate is multiply by the number of gene and the size of the population so that it represent the number of deletion per generation.
...
def optimum_distance(n_traits, alpha, Q, sigma_mut, duplication_rate, deletion_rate, mutation_rate, ratio, method, mutation_method, n_generations, list_init_dist):
    ...
    for d in list_init_dist :
        ...
        init_pos = init_pos*d 
        fgm = FisherGeometricModel(n_traits, init_pos, alpha, Q, sigma_mut, duplication_rate, deletion_rate, mutation_rate, ratio, method)
        # fgm.evolve(n_generations)
        fgm.evolve_successive(n_generations, mutation_method)
...
def ratio_gene_mutation(n_traits, initial_position, alpha, Q, sigma_mut, duplication_rate, deletion_rate, mutation_rate, method, mutation_method, n_generations, list_ratio):
    ...
    for prop in list_ratio :
        fgm = FisherGeometricModel(n_traits, initial_position, alpha, Q, sigma_mut, duplication_rate, deletion_rate, mutation_rate, prop, method)
        # fgm.evolve(n_generations)
        fgm.evolve_successive(n_generations, mutation_method)
...
def mutation_std(n_traits, initial_position, alpha, Q, duplication_rate, deletion_rate, mutation_rate, ratio, method, mutation_method, n_generations, list_sigma):
    ...
    for sig in list_sigma :
        fgm = FisherGeometricModel(n_traits, initial_position, alpha, Q, sig, duplication_rate, deletion_rate, mutation_rate, ratio, method)
        # fgm.evolve(n_generations)
        fgm.evolve_successive(n_generations, mutation_method)
...
def mutations_technics(n_traits, initial_position, alpha, Q, sigma_mut, duplication_rate, deletion_rate, mutation_rate, ratio, method, mutation_method, n_generations):
    ...
    for i in range(50) :
        ...
        fgm1 = FisherGeometricModel(n_traits, initial_position, alpha, Q, sigma_mut, 0, 0, mutation_rate, ratio, method) # Case with no rearrangement (Standard FGM)
        fgm1.evolve_successive(n_generations, mutation_method)
        results["No_Rearrangement"].append(fgm1.fitness)
        count += 1
        print("Test realized: ", count)

        fgm2 = FisherGeometricModel(n_traits, initial_position, alpha, Q, sigma_mut, duplication_rate, deletion_rate, mutation_rate, ratio, method) # Case with a mutation per generation affceting the whole genome
        fgm2.evolve_successive(n_generations, mutation_method)
...
if __name__ == "__main__" :
    ...
    duplication_rate = 10**(-2) # /gene/generation
    deletion_rate = 10**(-2) # /gene/generation
